...
const CartTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const resp = await axios.get(`http://localhost:8080/carts/${user.email}`);
      if (resp.status !== 200) {
  ...
  const handleCartDelete = async (cartId) => {
    const resp = await axios.delete(`http://localhost:8080/carts/${cartId}`);
    if (resp.status !== 200) {
      console.log(resp.data);
    } else {
      Swal.fire({
  ...
  const handleIncrease = async (cartId, quantity) => {
    const resp = await axios.put(`http://localhost:8080/carts/${cartId}`, {
      quantity: quantity + 1,
    });
    if (resp.status !== 200) {
      console.log(resp.data);
  ...
  return (
    <div className="container mx-auto w-10/12 overflow-x-auto mt-44">
      <div className="text-bold text-center text-5xl mb-32">
        <span>
      ...
      <div className="mt-24 mb-24 flex justify-around items-center">
        ...
        <div className="flex flex-col space-y-2">
          ...
          <span>Totla Price:{totalPrice.toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
